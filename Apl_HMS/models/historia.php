...
class Historia extends CI_Model {
  ...
  function __construct(){
    ...
    $this->arr_vaccunas = array(
      ...
      (object)array('codigo'=>'DPT','vacuna'=>'DPT MENORES DE 5 A�OS'),
      (object)array('codigo'=>'FAMARILLAN','vacuna'=>'FIEBRE AMARILLA NI�OS DE 1 A�O'),
      (object)array('codigo'=>'HEPATITA','vacuna'=>'HEPATITIS A '),
      (object)array('codigo'=>'HEPATITB','vacuna'=>'HEPATITIS B MENORES DE 1 A�O'),
      (object)array('codigo'=>'INFLUEZN','vacuna'=>'INFLUENZA NI�OS'),
      ...
      (object)array('codigo'=>'TDTT','vacuna'=>'TD O TT MUJERES EN EDAD F�RTIL 15 A 49 A�OS'),
      (object)array('codigo'=>'TRIPVIRAL','vacuna'=>'TRIPLE VIRAL NI�OS'),
    ...
    $this->arr_vacvalor = array(
      ...
      (object)array('codigo'=>'16','valor'=>'NO SE ADMINISTRA POR UNA TRADICI�N'),
      (object)array('codigo'=>'17','valor'=>'NO SE ADMINISTRA POR UNA CONDICI�N DE SALUD'),
      (object)array('codigo'=>'18','valor'=>'NO SE ADMINISTRA POR NEGACI�N DEL USUARIO'),
    ...
    $this->arr_tipoejecplan = array(
      (object)array("tipo"=>"ENFERMER�A"),
      (object)array("tipo"=>"TERAPIA F�SICA"),
    ...
    $this->arr_causaext = array(
      ...
      (object)array("causaext"=>"ACCIDENTE R�BICO"),
      (object)array("causaext"=>"ACCIDENTE OF�RICO"),
      (object)array("causaext"=>"OTRO TIPO DE ACCIDENTE"),
      (object)array("causaext"=>"EVENTO CATASTR�FICO"),
      (object)array("causaext"=>"LESI�N POR AGRESI�N"),
      (object)array("causaext"=>"LESI�N AUTOINFRENGIDA"),
      (object)array("causaext"=>"SOSPECHA DE MALTRATO F�SICO"),
    ...
    $this->arr_viaing = array(
      ...
      (object)array("viaing"=>"QUIR�FANO"),
      (object)array("viaing"=>"INTERNACI�N"),
      (object)array("destino"=>"HOSPITALZACI�N"),
      (object)array("destino"=>"URGENCIAS"),
      (object)array("destino"=>"OBSERVACION")
    );
    $this->arr_clastriage = array(
      (object)array("tipo"=>"1 URGENCIA VITAL"),
      (object)array("tipo"=>"2 INMEDIATA X EVOLUCION"),
      (object)array("tipo"=>"3 URGENCIA X MEDIDAS DIAGNOSTICAS"),
      (object)array("tipo"=>"4 URGENCIA NO COMPROMETIDA")
       );
    ...
    $this->arr_destinopac[]=(object)array("destino"=>"HOSPITALIZACI�N");
    $this->arr_destinopac[]=(object)array("destino"=>"OBSERVACION");
    ...
    $this->arr_udosis = array(
      (object)array("udos"=>"CENT�METROS C�BICOS"),
    ...
    //$this->db->where('estado_f_t67 IS NULL');
    $result = $this->db->get();
    ...
    function convenios($codigo)
  {
    ...
    if ($result != NULL) {
        $this->db->where('tercero_t95',$result[0]->nit_t70);
  ...
  function actualizar_ordenes()
  {   
      var_dump($this->input->post());
      $data = array('valunit_t67' => $this->input->post('valor'),'valor_t67' => $this->input->post('valor'));
      return $this->db->where('idhist_ordenes_t67',$this->input->post('orden'))->update('ps_hist_ordenes_t67', $data);
  }

  function actualizar_cantidad()
  {   
      var_dump($this->input->post());
      $data = array('cantidad_t67' => $this->input->post('valor'));
      return $this->db->where('idhist_ordenes_t67',$this->input->post('orden'))->update('ps_hist_ordenes_t67', $data);
  }
  ...
  function consulta_quirurgica($idhistoria,$numdorden){
    $this->db->where('id_historia_qui', $idhistoria);
    if (!empty($numdorden)) {
      $this->db->where('id_desquirurgica', $numdorden);
    }
    return $this->db->get('ps_des_quirurgica')->result();
  }
    /*PROCEDIMIENTOS QUIRURGICOS OFTALMOLOGICOS*/
  function consulta_procedimientos_q($idhistoria,$ordenes){
    $this->db->where('ps_historia_quirurgica', $idhistoria);
    $q['quirurgica'] = $this->db->get('ps_procedimiento_quirurgico_t13,ps_hist_ordenes_t67')->result();

    $this->db->where_in('idhist_ordenes_t67', $ordenes);
    $q['procedimmiento'] = $this->db->get('ps_hist_ordenes_t67')->result();

    return $q;
  }

  // Registrar consentimientos Informados
  public function registrarConsentimiento($idhistoria)
  {
    $consentimientos = (object)$this->input->post();
    $historia = $this->obtener($idhistoria);

    $arr_consentimiento['historia_t10'] = $idhistoria;
    $arr_consentimiento['idpaciente_t10'] = $historia->identificacion_t3;
    $arr_consentimiento['ps_objetivo_t10'] = $consentimientos->objetivos;
    $arr_consentimiento['ps_caracteristicas_t10'] = $consentimientos->caracteristica;
    $arr_consentimiento['ps_titulo_t10'] = $consentimientos->titulo_consentimiento;
    $arr_consentimiento['ps_conlusion_t10'] = $consentimientos->conclusion;
    $arr_consentimiento['ps_fecha_t10'] = $consentimientos->fechainf;
    $arr_consentimiento['ps_riesgos_cir_t10'] = $consentimientos->riesgos_cir;
    $arr_consentimiento['fmod_t10'] = $this->Modulo->ahora();
    $arr_consentimiento['usrmod_t10'] = $this->Modulo->usr->idusr;

    $this->db->insert('ps_consinformados_t10', $arr_consentimiento);
    $idcons = $this->db->insert_id();

          /*if(is_array($consentimientos->orden["procs"])){
          $idorden = $this->orden_siguiente();
          $orden = $consentimientos->orden["procs"];
          $num = count($orden);
          for($i=0;$i<$num;$i++){
            $arr_orden['tipo_t67']=$consentimientos->orden["procs"]["idtipoproce"];
            if(!empty($consentimientos->orden["procs"]["especialidades"])){
              $oespec = $this->Especialidades->obtener($consentimientos->orden["procs"]["especialidades"]);
              $arr_orden['idespecialidad_t67'] = $oespec->idps_especialidades_t9;
              $arr_orden['especialidad_t67'] = $oespec->especialidades_t9;
            }
            if(!empty($consentimientos->orden["procs"]["terceroid"])){
              $arr_terceros = $this->Modulo->objarrasoc($this->Constante->arr_terimxaliad,"idtercer");
              $tercero = $arr_terceros[$consentimientos->orden["procs"]["terceroid"]];
  ...
  // Listar las consentimientos informados
  function listarAutorizaciones($idhistoria,$id = ""){
    $this->db->where('historia_t10', $idhistoria);
    if (!empty($id)) {
      $this->db->where('psid_consinformados_t10', $id);
    }
    $q = $this->db->get('ps_consinformados_t10')->result();
    if (count($q) > 0) {
      return $q;
    }
  }
  ...
  function listarAutorizacion($idhistoria,$id = ""){
    $this->db->where('historia_t11', $idhistoria);
    if (!empty($id)){
      $this->db->where('psid_autorizaciones_t11', $id);
    }
      $this->db->where('ps_orden_t11 = idhist_ordenes_t67');
      $q = $this->db->get('ps_autorizaciones_t11,ps_hist_ordenes_t67')->result();
      if (count($q) > 0) {
        return $q;
    }
  }
  ...
  // Autorizaciones
  public function registrarAutorizaciones($idhistoria)
  {
    $consentimientos = (object)$this->input->post();
    $historia = $this->obtener($idhistoria);

    $arr_consentimiento['historia_t11'] = $idhistoria;
    $arr_consentimiento['idpaciente_t11'] = $historia->identificacion_t3;
    $arr_consentimiento['ps_info_solicitud_t11'] = $consentimientos->info_solicitud;
    $arr_consentimiento['ps_tipo_servicio_t11'] = $consentimientos->tipo_inca;
    $arr_consentimiento['ps_servicio_actual_t11'] = $consentimientos->servicio_actual;
    $arr_consentimiento['ps_especialidad_t11'] = $consentimientos->especialidad;
    $arr_consentimiento['ps_orden_t11'] = $consentimientos->orden;
    $arr_consentimiento['ps_cama_t11'] = $consentimientos->id_cama;
    $arr_consentimiento['ps_justificacion_t11'] = $consentimientos->jsutificacion;
    $arr_consentimiento['ps_observacion_t11'] = $consentimientos->observacion;
    $arr_consentimiento['fmod_t11'] = $this->Modulo->ahora();
    $arr_consentimiento['usrmod_t11'] = $this->Modulo->usr->idusr;

    $this->db->insert('ps_autorizaciones_t11', $arr_consentimiento);
    $idcons = $this->db->insert_id();
  }
    ...
    function quirurgica($idhistoria,$dathistoria="")
  {
    ...
    foreach ($dathistoria->principal as $f) {
      $this->db->where('idhist_ordenes_t67', $f);
      $this->db->update('ps_hist_ordenes_t67', array('ps_principal_t67' => "SI"));
    }
    foreach ($dathistoria->bilateral as $a) {
      $this->db->where('idhist_ordenes_t67', $a);
      $this->db->update('ps_hist_ordenes_t67', array('ps_bilateral_t67' => "SI"));
    }
    foreach ($dathistoria->realizado as $l) {
      $this->db->where('idhist_ordenes_t67', $l);
      $this->db->update('ps_hist_ordenes_t67', array('ps_realizado_t67' => "SI"));
    }

    foreach ($dathistoria->via as $k) {
      $this->db->where('idhist_ordenes_t67', $k);
      $this->db->update('ps_hist_ordenes_t67', array('via_t67' => "INTRAVENOSA"));
    }
    //$this->registar_procedimiento($dathistoria->orden["procs"],$idhistoria,$historia->identificacion_t3);
    $result = $this->modelo_universal->select("ps_des_quirurgica","*",array('id_historia_qui'=>$historia->idps_historia_t4));
    ...
    $arr_quirurgico['procedimientos_quir'] = $l;
    $arr_quirurgico['suspension'] = $dathistoria->obs;
    ...
    $arr_quirurgico['tipo_procedimiento'] = $dathistoria->tipo_procedimiento;
    $arr_quirurgico['tipo_cirugia_qui'] = $dathistoria->tipo_cirugia;
    $arr_quirurgico['ordenes'] = implode(',',$dathistoria->id_orden);
    $arr_quirurgico['fmod_quirurgica'] = $this->Modulo->ahora();
    $this->db->insert('ps_des_quirurgica',$arr_quirurgico);
  }

      public function psicologia($idhistoria)
      {
        $arr_evolucion = (object)$this->input->post();
        ...
        $arr_evoluciones["fmod_psicologia"] = $this->Modulo->ahora();
        $arr_evoluciones["usrmod_psicologia"]= $this->Modulo->usr->idusr;
        $arr_evoluciones['iden_usrmod'] = $this->Modulo->usr->identificacion_t0;
        $this->db->insert('ps_hist_evolucion_psicologia', $arr_evoluciones);
      }

      public function trabajo_social($idhistoria)
      {
        $arr_datos = (object)$this->input->post();
        var_dump($arr_datos->nombre_f[0]);
        
        $arr_data['idhistoria_t47'] = $idhistoria;
        $arr_data['nuclear_t47'] = $arr_datos->nuclear;
        $arr_data['extensa_t47'] = $arr_datos->extensa;
        $arr_data['recompuesta_t47'] = $arr_datos->recompuesta;
        $arr_data['m_cabeza_hogar_t47'] = $arr_datos->m_cabeza_hogar;
        $arr_data['p_cabeza_hogar_t47'] = $arr_datos->p_cabeza_hogar;
        $arr_data['f_ascenso_t47'] = $arr_datos->f_ascenso;
        $arr_data['f_homo_t47'] = $arr_datos->f_homo;
        $arr_data['g_paternnos_t47'] = $arr_datos->g_paternnos;
        $arr_data['plan_intervencion_t47'] = $arr_datos->plan_intervencion;
        $arr_data['observaciones_t47'] = $arr_datos->observaciones;
        $arr_data['seguimientos_t47'] = $arr_datos->seguimientos;
        $arr_data['fmod_t47'] = $this->Modulo->ahora();
        $arr_data['usrmod_t47'] = $this->Modulo->usr->idusr;
        $this->db->insert('ps_trabajo_social_t47', $arr_data);

        $num = count($arr_datos->nombre_f);
        for ($i=0; $i < $num; $i++) { 
          $arr_val['nombre_f_t48'] =  $arr_datos->nombre_f[$i];
          $arr_val['genero_t48'] =  $arr_datos->genero[$i];
          $arr_val['estado_civil_t48']  = $arr_datos->estado_civil[$i];
          $arr_val['parentesco_t48']  = $arr_datos->parentesco[$i];
          $arr_val['escolaridad_t48']  = $arr_datos->escolaridad[$i];
          $arr_val['ocupacion_t48'] =  $arr_datos->ocupacion[$i];
          $arr_val['idhistoria_t48']  = $idhistoria;
          $this->db->insert('familiares_trabajo_social_t48', $arr_val);
        }
      }
    ...
    //$consulta = $this->consulta_obtener($idhistoria);
        ...
        $arr_consulta["dxsecundario_t64"]=$dathistoria->dx["dxsecundario"] ;
        $arr_consulta["dxsecundariocod_t64"]=$dathistoria->dx["dxsecundariocod"] ;
        $arr_consulta["dxcuarto_t64"]=$dathistoria->dx["dxcuarto"]; 
        $arr_consulta["dxrelsecundario_t64"]=$dathistoria->dx["dxrelsecundario"]; 
        $arr_consulta["dxrelsecundariocod_t64"]=$dathistoria->dx["dxrelsecundariocod"] ;
        ...
        //Informaci�n de ps_hist_consulta_t64
        ...
        $arr_consulta["tipooption_t64"]=$dathistoria->dx["ojo_comprometido2"] ;
        $arr_consulta['causaext_t64'] = $dathistoria->dx['causaext'];
        $arr_consulta['idpaciente_t64'] = $dathistoria->dx['documento'];
        $arr_consulta['planmanejo_t64'] = $dathistoria->analisis['planmanejo'];
        $arr_consulta['conducta_t64'] = $dathistoria->analisis['conducta'];
        $arr_consulta['laboratorios_t64'] = $dathistoria->analisis['laboratorios'];
        ...
        $array["dx_tipodx_opto"]=$dathistoria->dx["tipodx"] ;
        $array["dx_tipodiag2_opto"]=$dathistoria->dx["tipodiag2"];
        $array["examen_motilidad_derecho"]=$dathistoria->revision["examen_motilidad_derecho"];
        if(!empty($resulta)){   
      ...
      if(is_array($dathistoria->antecedentes["PERSONALES"])){
        ...
        $resulta = $this->modelo_universal->select("ps_hist_antecedpatbiol_t66","*",array('idpaciente_t66'=>$historia->identificacion_t3));
        if(!empty($resulta)){   
        $this->db->where("idpaciente_t66",$historia->identificacion_t3); 
        $this->db->update('ps_hist_antecedpatbiol_t66', $arr_antec);
        }else{
        ...
        $resulta = $this->modelo_universal->select("antecedentes203","*",array('idpaciente203'=>$historia->identificacion_t3));
        if(!empty($resulta)){   
        $this->db->where("idpaciente203",$historia->identificacion_t3); 
        $this->db->update('antecedentes203', array('json203'=>json_encode($dathistoria)));
        }else{
  ...
  function oftalmologia($idhistoria,$dathistoria=""){
    ...
    //var_dump($dathistoria->dx['documento']);
      ...
      $arrt64['planmanejo_t64'] = $dathistoria->analisis['planmanejo'];
      $arrt64['conducta_t64'] = $dathistoria->analisis['conducta'];
      ...
      if(!empty($resulta)){   
        ...
        $array['agudeza_vp_con_correccion'] = $dathistoria->revision['agudeza_vp_con_correccion'];
        $array['agudeza__texto_vp_con_correccion'] = $dathistoria->revision['agudeza__texto_vp_con_correccion'];
        $array['agudeD'] = $dathistoria->revision['agudeD'];
        $array['agudeDD'] = $dathistoria->revision['agudeDD'];
        $array['agudeL'] = $dathistoria->revision['agudeL'];
        $array['agudeOD'] = $dathistoria->revision['agudeOD'];
        $array['agudeza_texto_con_correccion_OD'] = $dathistoria->revision['agudeza_texto_con_correccion_OD'];
        $array['agudeOI'] = $dathistoria->revision['agudeOI'];
        $array['agudeza_texto_con_correccion_OI'] = $dathistoria->revision['agudeza_texto_con_correccion_OI'];
        $array['agudeza_s_OD'] = $dathistoria->revision['agudeza_s_OD'];
        $array['agudeza_texto_con_correccion_s_OD'] = $dathistoria->revision['agudeza_texto_con_correccion_s_OD'];
        $array['agudeLOD'] = $dathistoria->revision['agudeLOD'];
        $array['agudeza_vp_con_correccion_OD'] = $dathistoria->revision['agudeza_vp_con_correccion_OD'];
        $array['agudeODs'] = $dathistoria->revision['agudeODs'];
        $array['agudeza_vp_sin_correccion_ODs'] = $dathistoria->revision['agudeza_vp_sin_correccion_ODs'];
        $array['agudeLs'] = $dathistoria->revision['agudeLs'];
        $array['agudeza_vp_con_correccion_s'] = $dathistoria->revision['agudeza_vp_con_correccion_s'];
        ...
        $array['examen_biomicroscopia_izquierdo'] = $dathistoria->revision['examen_biomicroscopia_izquierdo'];
        $array['examen_motilidad_derecho'] = $dathistoria->revision['examen_motilidad_derecho'];
        $array['examen_motilidad_izquierdo'] = $dathistoria->revision['examen_motilidad_izquierdo'];
        $array['examen_cristalino_derecho'] = $dathistoria->revision['examen_cristalino_derecho'];
        $array['examen_cristalino_izquierdo'] = $dathistoria->revision['examen_cristalino_izquierdo'];
        $array['examen_gonio_derecho'] = $dathistoria->revision['examen_gonio_derecho'];
        $array['examen_gonio_izquierdo'] = $dathistoria->revision['examen_gonio_izquierdo'];
...
function listar($buscado){
  ...
  function evol_psicologica($idhistoria)
  {
    $modelo = (object)$this->input->post();
    $this->db->where('id_historia',$idhistoria);
    if ($modelo->fecha_inicio) {
      $this->db->where('fmod_psicologia >=', $modelo->fecha_inicio);
      $this->db->where('fmod_psicologia <=',$modelo->fecha_final);
    }
    return $this->db->get('ps_hist_evolucion_psicologia')->result();
  }
  ...
  function registrar($id="",$dathistoria=""){
    ...
    function consulta_registrart($idhistoria,$dathistoria=""){
   ...
   /* var_dump($dathistoria);
    ...
    switch($dathistoria->formularioenviado){
      ...
      case "examingreso":
        ...
        if(is_null($dathistoria->estado_mental)===false)$arr_consulta['estado_mental_t64']=$dathistoria->estado_mental;
        if(is_null($dathistoria->salud_mental)===false)$arr_consulta['salud_mental_t64']=$dathistoria->salud_mental;
      ...
      case "evlucorden":
        ...
        break;
      case "ordenmedcs":
      		if ($this->db->database === "hms_UCI_LAGUITO") {
          	var_dump($orden);
          	exit();
          }
    ...
    if (is_null($dathistoria->hab_tabaquis)==false)$arr_consulta['hab_tabaquis_t64'] = $dathistoria->hab_tabaquis;
    if (is_null($dathistoria->hab_alcohol)==false)$arr_consulta['hab_alcohol_t64'] = $dathistoria->hab_alcohol;
    if (is_null($dathistoria->hab_sicofar)==false)$arr_consulta['hab_sicofar_t64'] = $dathistoria->hab_sicofar;
    if (is_null($dathistoria->hab_deport)==false)$arr_consulta['hab_deport_t64'] = $dathistoria->hab_deport;
    if (is_null($dathistoria->hab_tabaquis_text)==false)$arr_consulta['hab_tabaquis_t64_text'] = $dathistoria->hab_tabaquis_text;
    if (is_null($dathistoria->hab_alcohol_text)==false)$arr_consulta['hab_alcohol_t64_text'] = $dathistoria->hab_alcohol_text;
    if (is_null($dathistoria->hab_sicofar_text)==false)$arr_consulta['hab_sicofar_t64_text'] = $dathistoria->hab_sicofar_text;
    if (is_null($dathistoria->hab_deport_text)==false)$arr_consulta['hab_deport_t64_text'] = $dathistoria->hab_deport_text;
    $arr_consulta['medreg_t64']=$this->Modulo->usr->personal->registromedico_t10;
    $arr_consulta['medidentif_t64']=$this->Modulo->usr->identificacion_t0;
    $arr_consulta['mednomcomp_t64']=$this->Modulo->usr->nombre_t0;
    $arr_consulta['medcargo_t64']=$this->Modulo->usr->personal->cargo_t10;
...
// debug($dathistoria);
   /* var_dump($dathistoria);
    ...
    switch($dathistoria->formularioenviado){
        ...
        break;
      case "triage":
        $arr_consulta['motconsulta_t64']=$dathistoria->motconsulta;
        $arr_consulta['enfermactual_t64']=$dathistoria->enfermactual;
        $arr_consulta['triage_t64']=$dathistoria->triage;
        $arr_dathistoria['triage_t4']=$dathistoria->triage;
        $arr_consulta['altura_t64']=$dathistoria->altura;
        $arr_consulta['peso_t64']=$dathistoria->peso;
        $arr_consulta['imc_t64']=$dathistoria->imc;
        $arr_consulta['temp_t64']=$dathistoria->temp;
        $arr_consulta['pabd_t64']=$dathistoria->pabd;
        $arr_consulta['fr_t64']=$dathistoria->fr;
        $arr_consulta['fc_t64']=$dathistoria->fc;
        $arr_consulta['ta_t64']=$dathistoria->ta;
        $arr_consulta['pvc_t64']=$dathistoria->pvc;
        ///////nuevos campos de examen fisico 18/04/2018
        $arr_consulta['mcraneo_t64']=$dathistoria->mcraneo;
        $arr_consulta['mmuneca_t64']=$dathistoria->mmuneca;
        $arr_consulta['SINTR_t64']=$dathistoria->SINTR;
        $arr_consulta['SINTP_t64']=$dathistoria->SINTP;
        $arr_consulta['SINTF_t64']=$dathistoria->SINTF;
        $arr_consulta['pbrazo_t64']=$dathistoria->pbrazo;
        ///////
        $arr_consulta['sao2_t64']=$dathistoria->sao2;
        $arr_consulta['ggowmotora_t64']=$dathistoria->gmotora;
        $arr_consulta['ggowocular_t64']=$dathistoria->gocular;
        $arr_consulta['ggowverbal_t64']=$dathistoria->gverb;
        $arr_consulta['glsgow_t64']=$dathistoria->glsgow;
        $arr_dathistoria["estado_t4"]='ATENCION MEDICA';
      case "consultaso":
        $oConsultaso = $this->consultaso_obtener($idhistoria);
        if(is_array($dathistoria->consultaso)){
            ...
            if(!empty($orden["cum"][$i])){
              ...
              */
              /* datos modificados */
               ...
               $arr_orden['noposjust_t67'] = $this->input->post('justificacion');
              $arr_orden['ordennopost'] = $this->input->post('ordennopost');
              $arr_orden['noposevid_t67'] = $this->input->post('evidenciaCientifica');
              $arr_orden['noposprec_t67'] = $this->input->post('precaucion');
              $arr_orden['autorizado_m'] = $this->input->post('valido');
              $arr_orden['ejecucion_m'] = $this->input->post('valido2');
              //$arr_orden['dosis_t67'] = $this->input->post('dosis');
              $arr_orden['udosis_t67'] = $this->input->post('udosis');
              $arr_orden["usrmod_t67"]=$this->Modulo->usr->idusr;
              $arr_orden["fmod_t67"]=$this->Modulo->ahora();
              $arr_orden['id_consulta_t67']=$consulta->idhist_consulta_t64;
              $arr_orden['orden_t67']=$idorden;
    ...
    if(is_array($arr_evoluc)){
      ...
      $arr_evoluc["fmod_t68"]=$this->Modulo->ahora();
      if ($this->db->database == "hms_ERGOVIDA") {
        if (is_null($dathistoria->fechaO) == false)$arr_evoluc["fmod_t68"]=$dathistoria->fechaO;
      }
      $arr_evoluc['id_consulta_t68']=$consulta->idhist_consulta_t64;
      $arr_evoluc['idpaciente_t68']=$historia->identificacion_t3;
      $arr_evoluc['idhistoria_t68']=$historia->idps_historia_t4;
    ...
    $arr_dathistoria['r_caida'] =  $dathistoria->r_caida;
    $arr_dathistoria['r_abuso_sexual'] =  $dathistoria->r_abuso_sexual;
    $arr_dathistoria['r_auto_lesion'] =  $dathistoria->r_auto_lesion;
    $arr_dathistoria['r_hetero_agresion'] =  $dathistoria->r_hetero_agresion;
    $arr_dathistoria['r_fuga'] =  $dathistoria->r_fuga;
    $arr_dathistoria['alergia_medicamento'] =  $dathistoria->alergia_medicamento;
    $arr_dathistoria['r_biocontaminacion'] =  $dathistoria->r_biocontaminacion;
    $arr_dathistoria['r_ulcera'] =  $dathistoria->r_ulcera;
    //var_dump($arr_dathistoria); exit;
    $arr_dathistoria["usrmod_t4"]=$this->Modulo->usr->idusr;
    $arr_dathistoria["fmod_t4"]=$this->Modulo->ahora();
    $this->db->where("idps_historia_t4",$historia->idps_historia_t4);
...
//debug($inca);
    ...
    if(is_array($arrdetevol)){
      foreach($arrdetevol as $reg){
        ...
        switch($reg->categoria_t111){
          case "articulacion":
            $catdesc = "ARTICULACI�N";
            break;
          case "movimientos":
            $catdesc = "MOVIMIENTOS";
            break;
          case "mediosfisicos":
            $catdesc = "MEDIOS F�SICOS";
            break;
  ...
  function consulta_obtener($idhistoria){
    ...
    if ($modelo->fecha_inicio) {
      $this->db->where('fmod_t67 >=', $modelo->fecha_inicio);
      $this->db->where('fmod_t67 <=',$modelo->fecha_final);
    }
    ...
    if ($modelo->fecha_inicio) {
      $this->db->where('fmod_t68 >=', $modelo->fecha_inicio);
      $this->db->where('fmod_t68 <=',$modelo->fecha_final);
    }
    ...
    if ($modelo->fecha_inicio) {
      $this->db->where('fecha_t84 >=', $modelo->fecha_inicio);
      $this->db->where('fecha_t84 <=',$modelo->fecha_final);
    }
  ...
  function guardar_suministro(){
    ...
    $array_paciente = array(
                ...
                'archrips_t67' => $datos_paciente->valrips
              );
    return $this->db->insert('ps_hist_ordenes_t67', $array_paciente);
  }

  public function registrar_medicacion()
  {
    $arr_datos = (object)$this->input->post();
    $arr_registro['id_orden_registro'] = $arr_datos->orden;
    $arr_registro['id_historia_registro'] = $arr_datos->historia;
    $arr_registro['momento_200'] = $arr_datos->fechaInical;
    $arr_registro['usrmod'] = $this->Modulo->usr->idusr;
    $this->db->insert('registros_medicos_2018', $arr_registro);

    $date_fin = new DateTime();
    $date_fin->modify('+'.$arr_datos->tiempo.' hours');
    $arr_updpate['h_medicacion'] = $arr_datos->fechaInical;
    $arr_updpate['prox_medicacion'] = $date_fin->format($arr_datos->fechaInical);
    $arr_updpate['cantidadpen_t67'] = $arr_datos->cantidad_pe;
    $this->db->where('idhist_ordenes_t67', $arr_datos->orden);
    $this->db->update('ps_hist_ordenes_t67', $arr_updpate);
    return true;
  }

  public function obtener_registros($id)
  {
    $this->db->from("registros_medicos_2018");
    $this->db->where('id_historia_registro', $id);
    $query = $this->db->get();
    $arr_datos = $query->result();
    if (count($arr_datos) > 0){
      return $arr_datos;
  ...
  public function registrar_ingreso($idhistoria)
  {
    $arr_datos = (object)$this->input->post();

    $arr_evolucion['idhistoria'] = $idhistoria;
    $arr_evolucion['altura_evol'] = $arr_datos->altura;
    $arr_evolucion['peso_evol '] = $arr_datos->peso;
    $arr_evolucion['imc_evol'] = $arr_datos->imc;
    $arr_evolucion['fr_evol'] = $arr_datos->fr;
    $arr_evolucion['fc_evol'] = $arr_datos->fc;
    $arr_evolucion['ta_evol'] = $arr_datos->ta;
    $arr_evolucion['spo2_evol'] = $arr_datos->sao2;
    $arr_evolucion['tem_evol'] = $arr_datos->temp;
    $arr_evolucion['glasgow'] = $arr_datos->glsgow;
    $arr_evolucion['glasgow_motora'] = $arr_datos->gmotora;
    $arr_evolucion['glasgow_verbal'] = $arr_datos->gverb;
    $arr_evolucion['glasgow_ocular'] = $arr_datos->gocular;
    $arr_evolucion['nota_enfermeria'] = $arr_datos->plantratamiento;
    $arr_evolucion['usrmod_aux'] = $this->Modulo->usr->idusr;
    $arr_evolucion['fmod_aux'] = $arr_datos->fmod;

    return $this->db->insert('ps_evol_enfermeria_hms', $arr_evolucion);
  }

  public function obtener_evoluciones($id)
  {
    $modelo = (object)$this->input->post();
    $this->db->from("ps_evol_enfermeria_hms");
    if ($modelo->fecha_inicio) {
      $this->db->where('fmod_aux >=', $modelo->fecha_inicio);
      $this->db->where('fmod_aux <=',$modelo->fecha_final);
    }
